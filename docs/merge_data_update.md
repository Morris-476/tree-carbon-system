# 功能名稱
Arduino(ToF) / GNSS / 影片 時間對齊（純運算邏輯，透過資料上傳頁觸發）
...
- routes/api.py（新增 `POST /api/upload`，接收 GNSS/Arduino/影片檔案並回傳對齊結果）
...
2. **影片起始時間基準**：沿用先前確認的作法——以 Arduino(ToF) CSV 最早一筆時間戳記，作為
   影片第 0 影格的真實時間（不用 GNSS 最早時間戳記，原因是 GPS 定位延遲；不解析影片檔本身
   的 metadata）。
3. **觸發流程**：對應到網頁的「系統管理 → 數據管理維護 → 資料上傳」頁面
   （`templates/admin/upload.html`），管理員上傳 GNSS／Arduino／影片三個檔案，
...
| 函式 / API | 用途 | 輸入 | 輸出 |
|---|---|---|---|
| merge_data.align_sensor_data() | 對齊 Arduino(ToF) 與 GNSS 時間戳記，回傳合併結果 | arduino_path, gnss_path, video_filename, video_start_at, max_gap_seconds | dict（status, records, total_count, matched_gps_count, video_start_at, video_filename） |
| data_pipeline.run_sensor_time_sync() | 管線入口，包裝 align_sensor_data() | gnss_file_path, csv_file_path, video_path, video_start_at, max_gnss_gap_seconds | 同 align_sensor_data() |
| POST /api/upload | 接收上傳檔案並執行時間對齊 | multipart/form-data：gnss_file, arduino_file, mp4_file（選填） | `{success, total_count, matched_gps_count, video_start_at, video_filename, records}` 或 `{error}` |

## 合併/對齊邏輯摘要
- Arduino(ToF) CSV 前兩行是空白雜訊列，自動偵測 `Tree_ID` 開頭的表頭列再讀取。
- 以 Arduino 紀錄（每秒一筆）為主軸，用 `pandas.merge_asof`（最近時間、預設容忍 3 秒）
  抓最接近的 GNSS 紀錄；超過容忍範圍的那幾秒（例如 GNSS 尚未取得定位前）`latitude`/`longitude`
  會是 `null`，不會硬湊錯的座標。
- 兩份檔案都有的 `DATE`/`TIME` 欄位，合併後只保留一份 `recorded_at`（重複欄位收斂為 1 個）。
- GNSS 的 `INDEX` 欄位依需求整個捨棄，不出現在回傳結果中。
- 每筆紀錄額外算出 `video_offset_ms`（相對影片起始時間的毫秒數）與 `gnss_gap_ms`
  （配對到的 GNSS 紀錄時間差，供之後偵錯用）。
- 上傳的檔案會暫存到 `uploads/<隨機資料夾>/`（已在 `.gitignore` 排除，不進版控）。
- 用實際的 TREE_015.CSV（47 筆）與 01182401.CSV（33 筆）驗證過完整 `/api/upload` 流程：
  回傳 47 筆，其中 39 筆配對到 GPS 座標，前 8 筆（GNSS 尚未定位前）座標為 `null`，符合預期；
...
3. 上傳 GNSS 檔（如 `01182401.CSV`）、Arduino 檔（如 `TREE_015.CSV`），影片檔可選填。
4. 按「上傳辨識」，應顯示「時間對齊完成，共 N 筆，其中 M 筆配對到 GPS 座標」的提示。
5. 故意不選 GNSS 或 Arduino 檔案送出，應顯示對應的錯誤訊息（「請上傳 GNSS 檔案」等）。
