...
影像快門、ToF 感測器、GNSS 三者的時間對齊。

對齊策略：
  1. 以 ToF CSV 裡 flash_trigger=1 的行為「快門按下時刻」
  2. 在 GNSS 紀錄裡找時間戳記最接近快門時刻的那一筆（在容忍誤差內）
...
from services.gnss_parser import GnssRecord
...
class SyncResult:
    ...
    # 實際採用的 GNSS 紀錄與快門時刻的時間差（毫秒），供偵錯用
    gnss_gap_ms: int
...
def align_measurements(
    ...
    """
    對齊 ToF、GNSS、影片三者的時間戳記。

    Args:
        tof_records:              從 csv_parser 取得的 ToF 資料
        gnss_records:              從 gnss_parser 取得的 GNSS 資料
        video_start_timestamp_ms: 影片第 0 影格的 Unix 毫秒時間戳記
                                  ⚠️ 假設由呼叫端以 os.path.getctime() 換算，
                                     此假設待真實硬體驗證
        max_gap_ms:               GNSS 紀錄與快門時刻的最大容忍時間差（預設 200ms）

    Returns:
        SyncResult 對齊結果

    Raises:
        TimeSyncError: 對齊失敗（沒有 flash_trigger、GNSS 時間差過大等）
    """
    # TODO: 待實作 — 負責人：____
