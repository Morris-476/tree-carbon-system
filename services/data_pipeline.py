...
完整資料處理管線：從 GNSS / ToF / 影片 輸入，到資料庫寫入。

處理步驟（依架構文件）：
  ① csv_parser    — 解析 ToF CSV
  ② gnss_parser   — 解析 GNSS 文字檔
  ③ time_sync     — 對齊快門、ToF、GNSS 時間戳記
...
⚠️  2026/08/22 修改：原本模組頂層 import 的 services.gnss_parser / services.csv_parser /
...
def run_sensor_time_sync(
    ...
    max_gnss_gap_seconds: int = 3,
) -> dict:
    """
    資料處理管線的第①～③步：解析 Arduino(ToF) 與 GNSS 檔案、對齊時間戳記，回傳合併結果。
    純運算邏輯，不寫入資料庫。是 process_upload() 未來會呼叫的其中一段，
    目前先獨立提供，讓時間對齊功能不需等 tracker / diameter_calc 完成就能先運作。

    Args:
        gnss_file_path:        GNSS CSV 路徑
        ...
        max_gnss_gap_seconds:   GNSS 與 Arduino 紀錄的最大容忍時間差（秒）
    ...
    return merge_data.align_sensor_data(
        ...
        gnss_path=gnss_file_path,
        video_filename=video_path,
        video_start_at=video_start_at,
        max_gap_seconds=max_gnss_gap_seconds,
    )
...
def process_upload(
    ...
    max_gnss_gap_ms: int = 200,
) -> list[dict]:
    """
    主處理函式：解析感測器檔案、追蹤影片中的樹幹、計算樹徑與固碳量、寫入資料庫。

    Args:
        gnss_file_path:            GNSS 文字檔路徑（假設格式見 gnss_parser.py）
        ...
        max_gnss_gap_ms:           GNSS 對齊最大容忍時間差（毫秒）
